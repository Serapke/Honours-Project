...
clang++ -std=c++11 -S -emit-llvm test.cpp
clang++ -std=c++11 -S -emit-llvm heap-alloc.cpp -DDEBUG=false

# Link to a single bytecode file

~/build/bin/llvm-link -S -o single.ll test.ll functions.ll heap-alloc.ll
